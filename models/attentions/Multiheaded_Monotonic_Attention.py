...
class Multiheaded_Monotonic_Attention(nn.Module):
    def __init__(self, config,
                 ...
                 **kwargs):
        super(Multiheaded_Monotonic_Attention, self).__init__()
        ...
        self.step_act = config["step_act"].lower()
        self.step_bias = nn.Parameter(T.ones(1).float() * 20)
    ...
    def init_position(self):
        ...
        self.Wrho = nn.Parameter(T.randn(self.heads, self.d, 1))
    ...
    def score_positions(self, Q,
        ...
        rho = T.matmul(Q, self.Wrho.view(1, H, d, 1))
        assert rho.size() == (N, H, qS, 1)

        if self.step_act == "relu":
            steps = F.relu(rho)
        elif self.step_act == "sigmoid":
            steps = T.sigmoid(5 * rho)
        elif self.step_act == "controlled_relu":
            sb = T.sigmoid(self.step_bias)
            steps = sb * F.relu(rho) + (1 - sb) * T.sigmoid(5 * rho)
        elif self.step_act == "softstair":
            steps = self.softstair(rho)
        elif self.step_act == "free":
            steps = rho
        else:
            raise ValueError("step_act in config must be a name in [relu, sigmoid, controlled_relu, softstair, free]")


        mu = (steps * step_size) + past_attention
        ...
        return position_attention_scores, None, mu
