...
class GRUEncoderDecoder(nn.Module):
    def __init__(self, config):
        ...
        self.decodercell = nn.GRUCell(input_size=config["embd_dim"] + config["decoder_hidden_size"],
        ...
        self.output_map = TiedEmbedding(self.embed_layer.weight)
        self.eps = 1e-9
    ...
    def forward(self, src_idx, ptr_src_idx, max_oov_num, input_mask, trg_idx=None, output_mask=None):
        ...
        assert hn.size() == (self.k, N, self.encoder_hidden_size)
        ...
        for t in range(S2):
            ...
            h = self.decodercell(decoder_input, h)
