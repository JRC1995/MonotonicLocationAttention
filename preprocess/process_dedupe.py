...
def create_data(min_length, max_length, max_samples=10000):
    ...
    while len(total_samples) <= max_samples:
        ...
        for i in range(length):
            ...
            if flag == 0:
                Y.append(token)
        total_samples[" ".join(X + ["<eos>"])] = Y + ["<eos>"]
...
Path('../processed_data/dedup/').mkdir(parents=True, exist_ok=True)

train_save_path = Path('../processed_data/dedup/train.jsonl')
dev_save_path = {}
for key in dev_keys:
    dev_save_path[key] = Path('../processed_data/dedup/dev_{}.jsonl'.format(key))
test_save_path = {}
for key in test_keys:
    test_save_path[key] = Path('../processed_data/dedup/test_{}.jsonl'.format(key))
metadata_save_path = fspath(Path("../processed_data/dedup/metadata.pkl"))
